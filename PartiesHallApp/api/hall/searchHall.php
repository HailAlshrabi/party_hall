...
&& isset($_POST["endPrice"])

){
	
 $name = filterRequest("name");
 $date = filterRequest("date");
 $startPrice = filterRequest("startPrice");
 $endPrice = filterRequest("endPrice");

 $selectArray = array();
 array_push($selectArray, $name);
 array_push($selectArray, $date);
 array_push($selectArray, $startPrice);
 array_push($selectArray, $endPrice);

 $sqlSelect = "SELECT * ,(SELECT AVG(rating_star) FROM star_rating_hall WHERE hall_id = a.hall_Id) AS 'hall_rating_star' FROM hall_table AS a 
 WHERE hall_name LIKE ? 
 AND NOT EXISTS (SELECT * FROM booking_hall WHERE date_booking = ? )
 AND price >= ?
 AND price <= ?
